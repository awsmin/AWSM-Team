...
jQuery(document).ready(function($) {
	if($('#copy-awsm').length > 0) {
		var copyCode = new Clipboard('#copy-awsm');
    ...
    var selectoptions = $('.awsm-icon-select').select2({
            width: "100%",
            templateResult: awsm_formaticon,
            templateSelection: awsm_formaticon
    });

    $('#copy-awsm').on('click', function(e) {
    	event.preventDefault();
    });
    $('.awsm-add-row').on('click', function(e) {
        e.preventDefault();
    ...
    $('.awsm-members-list-selected').sortable({
        items: "li",
        axis: "y",
        containment: "parent",
    ...
    $(document).on('click', '.remove-member-to-list', function(e) {
    	e.preventDefault();
    	var member = $(this).data('member');
    	$('#awsm-members option[value="'+member+'"]').prop('disabled', false);
    	$(this).parent().remove();
    	awsm_select();
    });
    ...
    $('.awsm-sorable-table tbody').sortable({
        items: "tr",
        axis: "y",
        cursor: 'move',
    });

    $(document).on('change', "#awsm-members", function(el) {
        var memberlist = wp.template('awsm-member-list'),
    ...
    function awsm_formaticon(icon) {
        if (!icon.id) {
            return icon.text; }
        var $icon = $(
            '<span><i class="awsm-icon-' + icon.text.toLowerCase() + '"></i>  ' + icon.text + '</span>'
        );
        return $icon;
    }
